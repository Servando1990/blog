...
async def process_file(
    ...
    try:
        ...
        # Handle date format for Astro (ensure it's a proper date object or string without quotes)
        if "date" in post.metadata:
            ...
            if isinstance(date_val, str):
                try:
                    ...
                    post.metadata["date"] = parsed_date.strftime("%Y-%m-%d")
                except ValueError:
                    # If parsing fails, keep the original value
                    pass

        # Ensure published field exists for Astro
...
@app.command()
def main(
    root_dir: str = typer.Option("src/content", help="Root directory to process"),
